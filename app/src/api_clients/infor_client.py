...
class InforClient:
    ...
    def get_system_logs(self, last_check_time: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Infor system logs API error: {response.status_code}")
                return []
                
        except Exception as e:
            self.logger.error(f"Error fetching Infor system logs: {str(e)}")
            return []
    ...
    def get_audit_logs(self, last_check_time: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Infor audit logs API error: {response.status_code}")
                return []
    ...
    def _get_mock_system_logs(self) -> List[Dict[str, Any]]:
        ...
        try:
            mock_file = os.path.join(self.mock_data_path, 'infor_system_logs.json')
            ...
            self.logger.error(f"Error reading mock Infor system logs: {str(e)}")
            return []
    
    def _get_mock_audit_logs(self) -> List[Dict[str, Any]]:
        """Mock audit loglarını okur"""
        try:
            mock_file = os.path.join(self.mock_data_path, 'infor_audit_logs.json')
            with open(mock_file, 'r', encoding='utf-8') as f:
                logs = json.load(f)
